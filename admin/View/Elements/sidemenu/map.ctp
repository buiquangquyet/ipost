<div id="map" class="button_item"><!-- ▼ ウェブビューブロック -->
  <div class="frame_main_box box_kage">
    <div class="c"></div>
    <header class="panel-heading panel-title"><?php echo __('マップ'); ?></header>
    ...
    <div class="form-group">
        ...
        <div class="checkbox_box">
          <?php echo $this->Form->input("Sidemenu.map.enable", array('type' => 'radio', 'options' => Configure::read('SidemenuDisp'), 'label' => true, 'div' => false, 'legend' => false, 'style' => 'width:20px;margin-left:30px;margin-top:8px;', 'onchange' => 'postData("map_enable")')); ?>
        </div>
    ...
    <!-- ▼ 入力ボックス -->
    <div class="form-group">
      <?php echo $this->Form->create(false, array('type' => 'post', 'action' => 'regist', 'novalidate' => true, 'id' => 'map_form')); ?>
        <label for="coupon_html" class="form-label"><?php echo __('メニュー名'); ?></label>
        <?php echo $this->Form->input("Sidemenu.map.name", array('type' => 'text', 'class' => 'form-txt form-5', 'placeholder' => __('メニュー名の入力'), 'label' => false)); ?>

        <div class="button_box di ml_10 pt_0">
          <?php echo $this->Html->link(__('<i class="fa fa-edit mg-r-xs"></i>変更'), "javascript:postData('map_form');", array('escape' => false, 'class' => 'btn btn-info btn-sm')); ?>
        </div>
      <?php echo $this->Form->end(); ?>
      <div class="form_hr"></div>
    </div>
